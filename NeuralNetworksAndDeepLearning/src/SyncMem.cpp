...
void SyncMem::reshape(size_t size) {
	...
	_host_mem = new DATATYPE[_size];
	checkCudaErrors(Util::ucudaMalloc(&_device_mem, sizeof(DATATYPE)*_size));
}
...
void SyncMem::set_host_mem(const DATATYPE* mem) {
	set_mem(mem, HostToHost);
}

void SyncMem::set_device_mem(const DATATYPE* mem) {
	set_mem(mem, DeviceToDevice);
}
