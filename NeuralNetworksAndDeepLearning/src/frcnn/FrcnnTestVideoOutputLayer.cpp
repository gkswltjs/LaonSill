...
FrcnnTestVideoOutputLayer<Dtype>::FrcnnTestVideoOutputLayer(Builder* builder)
: Layer<Dtype>(builder),
...
void FrcnnTestVideoOutputLayer<Dtype>::testNet(vector<vector<Dtype>>& scores,
	...
	*/
	...
	const Dtype confThresh = Dtype(this->thresh);
	...
	*/
	...
	if (this->savePath == "") {
		...
		//cv::imwrite(this->savePath + "/" + to_string(this->fps++) + ".jpg", im);
	}
...
template <typename Dtype>
void FrcnnTestVideoOutputLayer<Dtype>::initialize() {
