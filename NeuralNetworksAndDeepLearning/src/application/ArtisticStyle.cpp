...
void ArtisticStyle::style(const char* content_img_path, const char* style_img_path,
	...
	network->feedforward(d_content, end);
	checkCudaErrors(cudaFree(d_content));

	DATATYPE *content_out = new DATATYPE[dstLayerOutSize];
	//checkCudaErrors(cudaMalloc(&d_content_out, sizeof(DATATYPE)*dstLayerOutSize));
	checkCudaErrors(cudaMemcpyAsync(content_out, dstLayer->getOutput(), sizeof(DATATYPE)*dstLayerOutSize, cudaMemcpyDeviceToHost));
	Util::printData(content_out, dstLayerOutDim.rows, dstLayerOutDim.cols, 3, 1, "content_out:");
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	network->feedforward(d_style, end);
	checkCudaErrors(cudaFree(d_style));

	DATATYPE *style_out = new DATATYPE[N*N];
	DATATYPE *style_temp_out = new DATATYPE[dstLayerOutSize];
	checkCudaErrors(cudaMemcpyAsync(style_temp_out, dstLayer->getOutput(), sizeof(DATATYPE)*dstLayerOutSize, cudaMemcpyDeviceToHost));
	Util::printData(style_temp_out, dstLayerOutDim.rows, dstLayerOutDim.cols, dstLayerOutDim.channels, 1, "style_temp_out:");
	gramMatrix(style_temp_out, N, M, style_out);
	//Cube<DATATYPE> style_out_arma(style_out, 1, N, N);
	//style_out_arma.print("style_out_aram:");
	Util::printData(style_out, N, N, 1, 1, "style_out:");
	...
	DATATYPE *content_loss = new DATATYPE[dstLayerOutSize];
	DATATYPE *d_content_loss;
	checkCudaErrors(cudaMalloc(&d_content_loss, sizeof(DATATYPE)*dstLayerOutSize));
	...
	while(true) {
		...
		network->feedforward(d_input, end);

		// compute content loss
		checkCudaErrors(cudaMemcpyAsync(content_loss, dstLayer->getOutput(), sizeof(DATATYPE)*dstLayerOutSize, cudaMemcpyDeviceToHost));

#ifdef STYLE_
		// content loss를 변경하기전에 style loss부터 계산.
		DATATYPE* style_loss = new DATATYPE[N*N];
		gramMatrix(content_loss, N, M, style_loss);
		Util::printData(style_loss, N, N, 1, 1, "style_loss:");
		Util::printData(style_out, N, N, 1, 1, "style_out:");
		// G-A
		for(int i = 0; i < N*N; i++) {
			style_loss[i] -= style_out[i];
		}

		// transpose(F)*(G-A)
		//Util::setPrint(true);
		Util::printData(style_loss, N, N, 1, 1, "G-A:");
		Util::printData(content_loss, dstLayerOutDim.rows, dstLayerOutDim.cols, dstLayerOutDim.channels, 1, "F:");
		DATATYPE* content_loss_rmo = new DATATYPE[dstLayerOutSize];
		for(int channel = 0; channel < dstLayerOutDim.channels; channel++) {
			for(int row = 0; row < dstLayerOutDim.rows; row++) {
				for(int col = 0; col < dstLayerOutDim.cols; col++) {
					content_loss_rmo[row+col*dstLayerOutDim.rows+channel*M] = content_loss[col+row*dstLayerOutDim.cols+channel*M];
				}
			}
		}
		//Util::printData(content_loss_rmo, dstLayerOutDim.rows, dstLayerOutDim.cols, dstLayerOutDim.channels, 1, "F_temp:");
		Cube<DATATYPE> F_temp(content_loss_rmo, 1, M, N);
		Mat<DATATYPE> G_A(style_loss, N, N);
		Mat<DATATYPE> F;
		...
		Util::printData(result.mem, N, M, 1, 1, "F_temp:");
		...
		DATATYPE* style_delta_temp = new DATATYPE[N*M];
		//DATATYPE* style_delta = new DATATYPE[N*M];
		const float coef = beta/(N*M);					// too small coef.
		cout << "coef: " << coef << endl;
		result *= coef;

		for(int channel = 0; channel < dstLayerOutDim.channels; channel++) {
			for(int m = 0; m < M; m++) {
				int row = m % dstLayerOutDim.rows;
				int col = m / dstLayerOutDim.rows;
				style_delta_temp[col+row*dstLayerOutDim.cols+channel*M] =
						result.mem[m*N+channel];
			}
		}
		Util::printData(style_delta_temp, dstLayerOutDim.rows, dstLayerOutDim.cols, dstLayerOutDim.channels, 1, "style_delta_temp:");
		...
		for(int i = 0; i < N*M; i++) {
			if(content_loss[i] > 0) content_loss[i] -= content_out[i];
			else content_loss[i] = 0;


			content_loss[i] = alpha*content_loss[i];// + style_delta_temp[i];
#ifdef STYLE_
			// style delta + content delta
			content_loss[i] += style_delta_temp[i];
#endif
		}

		Util::printData(content_loss, dstLayerOutDim.rows, dstLayerOutDim.cols, 3, 1, "content_loss:");
		checkCudaErrors(cudaMemcpyAsync(d_content_loss, content_loss, sizeof(DATATYPE)*dstLayerOutSize, cudaMemcpyHostToDevice));

		// back propagation 전에 error들을 합해준다.

		// back propagate content loss
		dstLayer->backpropagation(0, d_content_loss);
		...
		process_disp.resize(temp_src, true).display(temp_src);
