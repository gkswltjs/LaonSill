...
template void GetPriorBBoxes(const float* priorData, const int numPriors,
		vector<NormalizedBBox>* priorBBoxes, vector<vector<float>>* priorVariances);
...
void GetLocPredictions(const Dtype* locData, const int num, const int numPredsPerClass,
		const int numLocClasses, const bool shareLocation, vector<LabelBBox>* locPreds) {
	locPreds->clear();
	if (shareLocation) {
 ...
 * allMatchOverlaps
 */
void FindMatches(const vector<LabelBBox>& allLocPreds,
		const map<int, vector<NormalizedBBox>>& allGtBBoxes,
		const vector<NormalizedBBox>& priorBBoxes,
		const vector<vector<float>>& priorVariances,
		const int numClasses, const bool shareLocation, const string& matchType,
		const float overlapThreshold, const bool usePriorForMatching,
		const int backgroundLabelId, const string& codeType,
		const bool encodeVarianceInTarget, const bool ignoreCrossBoundaryBBox,
		vector<map<int, vector<float>>>* allMatchOverlaps,
		vector<map<int, vector<int>>>* allMatchIndices) {
	...
	for (int i = 0; i < num; i++) {
		...
		map<int, vector<int>> matchIndices;
		map<int, vector<float>> matchOverlpas;
		...
		const vector<NormalizedBBox>& gtBBoxes = allGtBBoxes.find(i)->second;
		...
		if (!usePriorForMatching) {
			for (int c = 0; c < locClasses; c++) {
				...
				vector<NormalizedBBox> locBBoxes;
				bool clipBBox = false;
			...
			vector<int> tempMatchIndices;
			vector<float> tempMatchOverlaps;
			const int label = -1;
...
void DecodeBBox(const NormalizedBBox& priorBBox, const vector<float>& priorVariances,
	...
	if (codeType == "CORNER") {
		...
		SASSERT(false, "Unknown LocLossType: %s", codeType.c_str());
	}
...
void MatchBBox(const std::vector<NormalizedBBox>& gtBBoxes,
	...
	// 해당 index와 overlap을 추가 업데이트.
		...
		SASSERT(false, "Unknown matching type: %s", matchType.c_str());
	}
	return;
}
...
inline bool IsEligibleMining(const std::string& miningType, const int matchIdx,
		const float matchOverlap, const float negOverlap) {
	if (miningType == "MAX_NEGATIVE") {
		return matchIdx == -1 && matchOverlap < negOverlap;
	} else if (miningType == "HARD_EXAMPLE") {
		return true;
	} else {
		return false;
	}
...
int CountNumMatches(const std::vector<std::map<int, std::vector<int>>>& allMatchIndices,
	...
	int numMatches = 0;
	for (int i = 0; i < num; i++) {
		const map<int, vector<int>>& matchIndices = allMatchIndices[i];
...
void ApplyNMS(const vector<NormalizedBBox>& bboxes, const vector<float>& scores,
	...
	while (scoreIndexVec.size() != 0) {
		...
		for (vector<pair<float, int>>::iterator it = scoreIndexVec.begin();
...
void ApplyNMS(const bool* overlapped, const int num, std::vector<int>* indices) {
	...
	while (indexVec.size() != 0) {
		...
		for (vector<int>::iterator it = indexVec.begin(); it != indexVec.end(); ) {
			int curIdx = *it;
...
void ComputeConfLossGPU(Data<Dtype>& confData, const int num,
	...
	int intLossType = 0;
	if (lossType == "SOFTMAX") intLossType = 0;
	else if (lossType == "LOGISTIC") intLossType = 1;
	else SASSERT0(false);
	...
	for (int i = 0; i < num; i++) {
		vector<float> confLoss(lossData, lossData + numPredsPerClass);
...
void MineHardExamples(Data<Dtype>& confData,
	...
	*numNegs = 0;
	...
	// [num][num_priors]
	vector<vector<float>> allConfLoss;
	...
	// batch내 이미지별 prior box 전체에 대한 loss 리스트.
	// [num][num_priors]
	vector<vector<float>> allLocLoss;
	if (miningType == "HARD_EXAMPLE") {
		...
		for (int i = 0; i < num; i++) {
			vector<float> locLoss(numPriors, 0.f);
			allLocLoss.push_back(locLoss);
		}
	}

	for (int i = 0; i < num; i++) {
		map<int, vector<int>>& matchIndices = (*allMatchIndices)[i];
		const map<int, vector<float>>& matchOverlaps = allMatchOverlaps[i];
		// loc + conf loss.
		const vector<float>& confLoss = allConfLoss[i];
		const vector<float>& locLoss = allLocLoss[i];
		vector<float> loss;
		...
		for (map<int, vector<int>>::iterator it = matchIndices.begin();
			...
			vector<pair<float, int>> lossIndices;
			...
			if (hasNmsParam && nmsThreshold > 0) {
				...
				vector<float> selLoss;
				vector<NormalizedBBox> selBBoxes;
				if (usePriorForNms) {
					for (int m = 0; m < matchIndices[label].size(); m++) {
						if (IsEligibleMining(miningType, matchIndices[label][m],
								matchOverlaps.find(label)->second[m], negOverlap)) {
...
void EncodeLocPrediction(const std::vector<LabelBBox>& allLocPreds,
	...
	for (int i = 0; i < num; i++) {
		for (map<int, vector<int>>::const_iterator it = allMatchIndices[i].begin();
				it != allMatchIndices[i].end(); it++) {
			const int label = it->first;
			const vector<int>& matchIndex = it->second;
			SASSERT0(allLocPreds[i].find(label) != allLocPreds[i].end());
			const vector<NormalizedBBox>& locPred = allLocPreds[i].find(label)->second;
...
void EncodeBBox(const NormalizedBBox& priorBBox, const vector<float>& priorVariance,
		const std::string& codeType, const bool encodeVarianceInTarget,
...
void ComputeLocLoss(Data<Dtype>& locPred, Data<Dtype>& locGt,
		const vector<map<int, vector<int>>>& allMatchIndices, const int num,
		const int numPriors, const std::string& locLossType,
		vector<vector<float>>* allLocLoss) {
...
void EncodeConfPrediction(const Dtype* confData, const int num, const int numPriors,
	...
	bool doNegMining = (miningType != "NONE");
	int count = 0;
	for (int i = 0; i < num; i++) {
		if (allGtBBoxes.find(i) != allGtBBoxes.end()) {
			...
			for (map<int, vector<int>>::const_iterator it = matchIndices.begin();
				...
				for (int j = 0; j < numPriors; j++) {
					...
					if (confLossType == "SOFTMAX") {
						confGtData[idx] = gtLabel;
					} else if (confLossType == "LOGISTIC") {
